...
namespace expresspp {
	...
	static const std::map<std::string, std::string> _EXT_MIME_MAP =
	{
	...
	static const std::map<int, std::string> _STATUS_CODE_DESCRIPTION =
	{
		{ 100, "Continue" },
	...
	inline std::string StatusMessage(int status_code)
	{
	...
	inline std::string HeaderDateStr()
	{
		std::string str;
    ...
    inline std::vector<std::string> StringSplit(const std::string & str, char sep)
    {
    ...
    inline std::map<std::string, std::string> ParseQueryString(std::string queryStr){
        std::map<std::string, std::string> queryMap;

        auto queryItems = StringSplit(queryStr, '&');
        for (auto queryItem : queryItems){
            auto keyValues = StringSplit(queryItem, '=');
            if (keyValues.size() == 2){
                queryMap[keyValues[0]] = keyValues[1];
            }
        }
